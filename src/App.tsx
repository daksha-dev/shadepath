...
import { fallbackUv, type UserProfile, type UvWeather } from './utils/uvDose'
import './index.css'
...
function App() {
  ...
  useEffect(() => {
    fetchBengaluruUv().then(setWeather)
  }, [])

  const journey = journeys.find((item) => item.id === journeyId) ?? journeys[0]
  const routes = useMemo(
    () => scoreRoutes(journey.routes, mode, profile, weather, departureOffset),
    [journey.routes, mode, profile, weather, departureOffset],
  )

  const selectedRoute = routes.find((route) => route.id === selectedRouteId) ?? routes[0]
  const nowRoute = useMemo(() => scoreRoutes(journey.routes, mode, profile, weather, 0).find((route) => route.id === selectedRoute.id), [
    journey.routes,
    mode,
    profile,
    selectedRoute.id,
  ...
  const safest = safestDepartureInsight(journey.routes, mode, profile, weather, selectedRoute.id)
    ...
    <div className="app-shell">
      <header className="app-header">
        ...
        <div className="header-metrics">
          <span><Activity size={18} /> Live UV {weather.uvIndex.toFixed(1)}</span>
          <span>{weather.source === 'Open-Meteo' ? 'Live UV source: Open-Meteo' : 'Using demo UV data'}</span>
        </div>
      </header>
        ...
        <main className="planner-layout">
          ...
          <aside className="right-rail">
            <section className="uv-card">
              <p className="eyebrow">Open-Meteo snapshot</p>
